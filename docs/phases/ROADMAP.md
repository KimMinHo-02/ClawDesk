...
| Phase | 이름 | 요약 | 상태 |
| --- | --- | --- | --- |
| 0 | Product / Harness | 제품 계약, security invariants, 아키텍처, OpenCode harness(agent/command), 최소 scaffold | in progress |
| 1 | Windows Environment + OpenClaw Adapter | Windows 버전/아키텍처 detect, Node detect, OpenClaw executable/version/gateway/update status, ProcessRunner, fake CLI contract test | not started |
